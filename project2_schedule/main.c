#include <stdio.h>
#include <stdlib.h>
#include <string.h>
#include "schedule.h"
#include "linkedList.h"

/* run this program using the console pauser or add your own getch, system("pause") or input loop */

int main(int argc, char *argv[]) {
	
	int exit_flag = 0;
	FILE *fp;
	
	char name[200];
	char place[100], typeName[100];	
	size_t size;
	int month;
	int day;
	int type;
	void *list;
	void *ndPtr;
	void *schedInfo;
	int option;
	int cnt;
	


	//1. FILE pointer open & error handling
	
	if((fp= fopen("linkedList.h","r"))==NULL)//파일이 열리지 않았을 시 오류 메세지 출력. 
	{
		printf("invalid \n");//오류 메세지 출력. 
		return -1;
	}


	//initializing the list
	printf("Reading the data files... \n");//파일이 정상적으로 실행 되었을 경우. 
	list = (void*)list_genList();//파일을 리스트로 읽어온다. 
	
		
	//2. read from the file
	while (( fp= fopen("linkedList.h","r"))!=NULL )//파일 읽을 수 있는 상태일 시  
	{	
	
		//fill code here -- generate genSchedInfo structure by genSchedInfo function
		typedef struct genSchedInfo{//genSchedInfo 구조체 생성 함수. 5가지 정보 포함하고있다. 
		char name[200];//이름 
		char place[100]; 	//장소 
		int month;//월 
		int day;//일 
		int type;//타입 
		};
		
		//put the scheduling info instance generated by genSchedInfo function
		
		list_addTail(schedInfo, list);//리스트에 추가한다. 
	}
	
	fclose(fp);//파일 포인터 닫음!. 

	
	printf("Read done! %i schedules are read\n", list_len(list));//파일 포인터 닫힘을 출력한다. 
	
	
	//program starts
	while(exit_flag == 0) 
	{
		//3. menu printing 
		printf("1. print all the schedules.\n");//메뉴판을 출력하는 프린트문. 
		printf("2. search for schedules in the month.\n");
		printf("3. search for schedules in the place.\n");
		printf("4. search for specific type schedule\n");
		printf("5. exit! \n");
		
		//4. get option from keyboard
		
		printf("select an option!:");//옵션 선택을 유도한다. 
		scanf("%i",&option);//옵션을 선택받는다. 

		
		switch(option)
		{
			case 1: //print all the schedules
				printf("printing all the schedules in the scheduler.....\n\n\n");
				
				ndPtr = list;
				while (list_isEndNode(ndPtr) == 0)
				{
					//file code here -- print count and each scheduling info element
	
					ndPtr = list_getNextNd(ndPtr); //get the next node from the list
					schedInfo = list_getNdObj(ndPtr); //get the object (scheduling info)
					
					//fill code this part - end
				}
				
				break;
				
			case 2:
				printf("which month ? : ");
				scanf("%i", &month);
				
				ndPtr = list;
				while (list_isEndNode(ndPtr) == 0)
				{
					//file code here -- print scheduling info elements matching to the month
					ndPtr = list_getNextNd(ndPtr); //get the next node from the list
					schedInfo = list_getNdObj(ndPtr); //get the object (scheduling info)
					
					//fill code this part - end
				}
				
				break;
				
			case 3:
				printf("which place ? : ");
				scanf("%s", place);
				
				ndPtr = list;
				while (list_isEndNode(ndPtr) == 0)
				{
					//file code here -- print scheduling info elements matching to the place
					ndPtr = list_getNextNd(ndPtr); //get the next node from the list
					schedInfo = list_getNdObj(ndPtr); //get the object (scheduling info)
					
					//fill code this part - end
				}
				
				break;
				
			case 4:
				printf("which type ?\n");
				sched_printTypes();
				printf("your choice : ");
				scanf("%s", typeName);
				
				if ((list_isEndNode(ndPtr) != NULL/* fill code here -- convert the type and check if the type is valid */))
				{
					sched_convertType(typeName);//변환 코드 
					ndPtr = list;
					while (list_isEndNode(ndPtr) == 0)
					{
						//file code here -- print scheduling info elements matching to the place
						ndPtr = list_getNextNd(ndPtr); //get the next node from the list
						schedInfo = list_getNdObj(ndPtr); //get the object (scheduling info)
						
						//fill code this part - end
					}
				}
				else
				{
					printf("wrong type name!\n");
				}
				break;
				
			case 5:
				printf("Bye!\n\n");
				exit_flag = 1;
				break;
				
			default:
				printf("wrong command! input again\n");
				break;
		}
		
		
	}
	
	return 0;
}
